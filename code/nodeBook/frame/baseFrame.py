...
class BaseFrame:
    ...
    def refreshCanvas(self,pageKey:str=None):
        ...
        scrollCanvas.create_window(
            0,
            0,
            width=self.getConfig().getContentPageWidth()+self.getPage().resizeWidthOffset(),
            window=contentFrame,
            anchor=tk.NW
        )
    ...
    def cacheThread(self,func,key:str,args:tuple=()):
        """缓存线程并执行"""
        self.getThreadController().cacheThread(threading.Thread(target=func,args=args,daemon=True),key)