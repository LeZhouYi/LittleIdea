...
class WidgetController():
    ...
    def repackWidget(self,keys:list[str])->None:
        """重新安排布局"""
        for key in keys:
    ...
    def cacheWidget(self,widget:tk.Widget,parentKey:str,key:str,cnf:dict)->None:
        """缓存控件"""
        if cnf!=None:
    ...
    def cacheWidgetByGrid(self,widget:tk.Widget,parentKey:str,key:str,cnf:dict)->None:
        """缓存控件"""
        if cnf!=None:
            widget.grid(cnf)
        self.__cacheWidget(widget,key)
    ...
    def __getChilds(self,key:str)->list:
        """获取子控件关键字"""
        if key not in self.relations:
            raise Exception("键名%s不存在"%key)
        return self.relations[key]["childs"]
    ...
    def __cacheWidget(self,widget:tk.Widget,key:str):
        """将控件添加进缓存池"""
