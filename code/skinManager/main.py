...
class MainFrame:
    def __init__(self):
        ...
        self.loadRoleListContent() #加载角色列表
    ...
    def __initBaseData(self):
        ...
        self.mainWindow.bind(Event.Escape,self.escBack)
...
#######################具体布局################################
    ...
    def initRoleListFrame(self):
        ...
        contentPageCanvas.config(yscrollcommand=contentPageScroll.set,yscrollincrement=5)
    ...
    def loadRoleListContent(self):
        ...
        roleIndex = 0
        rolePageWidth = contentFrame.winfo_width() if contentFrame.winfo_width()>self.config.getWindowWidth() else self.config.getWindowWidth() #当前内容页宽度
    ...
    def loadSkinListContent(self,key:str):
        ...
        skinIndex = 0
        coloumnWidth = contentPageCanvas.winfo_width()
        coloumnMax = self.config.getSkinColoumMax(coloumnWidth)
        skinFrameBorder = self.config.getSkinBorder(coloumnWidth,coloumnMax)
        for fileDir in os.listdir(rolePath):
            ...
            if utils.isPathExist(filePath):
                for image in self.getSkinImages(filePath):
                    rowIndex = (skinIndex // coloumnMax) + 1 #计算grid布局的行列
                    columnIndex = (skinIndex % coloumnMax) + 1

                    skinSingleFrame = tk.Frame(contentPageFrame,border=skinFrameBorder,cnf=self.widgetCnf()) #单个皮肤
                    ...
                    skinSingleFrame.bind(Event.MouseWheel,utils.eventAdaptor(self.scrollVertical,widget=contentPageCanvas))
                    skinSingleBtn.bind(Event.MouseWheel,utils.eventAdaptor(self.scrollVertical,widget=contentPageCanvas))
                    skinSingleText.bind(Event.MouseWheel,utils.eventAdaptor(self.scrollVertical,widget=contentPageCanvas))
                    ...
                    self.sourceController.cacheImage(image,"default_%d"%skinIndex,"skinList")
                    skinIndex+=1
    ...
    def loadSkinControlPanel(self,key:str):
        ...
        controlNowText = tk.Label(controlNowFrame,text=self.getModsUseSkinText(self.selectRoleKey),cnf=self.textCnf())
        ...
        controlNowDelete.bind(Event.MouseLefClick,self.clickDeleteModSkin)
        controlCatchBtn.bind(Event.MouseLefClick,self.clickCreateSkinImage)
...
#######################控件事件################################
    def clickUpdateConfig(self,event,btn: tk.Button):
        ...
        self.cacheThread(threading.Thread(target=self.replaceText,args=(btn, "更新"),daemon=True),None)
    ...
    def clickReplaceSkin(self,event):
        ...
        if controlReplaceBtn != None and controlReplaceBtn["text"] == "替换":
            ...
            if utils.isEmpty(pathName):
                ...
                self.cacheThread(threading.Thread(target=self.replaceText, args=(controlReplaceBtn, "替换"), daemon=True),None)
                return
    ...
    def clickDeleteModSkin(self, event):
        """点击删除mods路径对应角色的皮肤文件"""
        deleteBtn = self.getWidget("controlNowDelete")
        if utils.isWidget(deleteBtn, tk.Button) and deleteBtn["text"] == "删除":
            useModsLabel = self.getWidget("controlNowText")
            if utils.isWidget(useModsLabel, tk.Label):
                modPath = self.config.getModsPath()
                if utils.isEmpty(modPath):
                    deleteBtn.config(text="未选择Mods路径", fg=self.config.getColorFail())
                    self.cacheThread(threading.Thread(
                        target=self.replaceText, args=(deleteBtn, "删除"), daemon=True
                    ),None)
                    return
                elif not os.path.exists(modPath):
                    deleteBtn.config(text="Mods路径不存在", fg=self.config.getColorFail())
                    self.cacheThread(threading.Thread(
                        target=self.replaceText, args=(deleteBtn, "删除"), daemon=True
                    ),None)
                    return
                ...
                if (
                    ...
                    deleteBtn.config(text="未有使用皮肤", fg=self.config.getColorFail())
                    self.cacheThread(threading.Thread(
                        target=self.replaceText, args=(deleteBtn, "删除"), daemon=True
                    ),None)
                    return
    ...
    def clickCreateSkinImage(self, event):
        """点击制作预览皮肤"""
        selectLabel = self.getWidget("controlSelectText")
        createReviewBtn = self.getWidget("controlCatchBtn")
        if utils.isWidget(selectLabel, tk.Label) and utils.isWidget(
            createReviewBtn, tk.Button
        ):
            if createReviewBtn["text"] != "截图制作预览":
                return
            createReviewBtn.config(text="处理中", fg=self.config.getColorDefault())
            selectSkinName = selectLabel["text"]  # 选择皮肤名称
            skinSourcePath = self.config.getSkinPath()  # 皮肤源路径
            if not utils.isPathExist(skinSourcePath):
                createReviewBtn.config(text="皮肤路径不存在", fg=self.config.getColorFail())
                self.cacheThread(threading.Thread(
            ...
            if not utils.isPathExist(skinPath):
                ...
                return
            createReviewBtn.config(text="截图中", fg=self.config.getColorDefault())
            tempPath = os.path.join(os.getcwd(), self.config.getTempDir())
            ...
            next(generater)
            createReviewBtn.config(text="制作预览中", fg=self.config.getColorDefault())
            ...
            createReviewBtn.config(text="操作成功", fg=self.config.getColorSuccess())
            self.cacheThread(threading.Thread(target=self.loadSkinListPage,args=(self.selectRoleKey,),daemon=True),"content")

    def deleteModSkin(self, deleteBtn: tk.Button, skinPath: str, useLabel: tk.Label):
        """删除正在使用的皮肤"""
        deleteBtn.config(text="删除中...", fg=self.config.getColorDefault())
        shutil.rmtree(skinPath)
        useLabel.config(text="")
        deleteBtn.config(text="删除成功", fg=self.config.getColorSuccess())
        self.cacheThread(threading.Thread(target=self.replaceText, args=(deleteBtn, "删除"), daemon=True),None)
    ...
    def replaceSkin(self, modsPath: str, pathName: str) -> bool:
        ...
        if utils.isWidget(replaceBtn, tk.Button) and os.path.exists(skinPath):
            ...
            self.cacheThread(threading.Thread(target=self.replaceText, args=(replaceBtn, "替换"), daemon=True),None)
            modUseRoleText = self.getWidget("controlNowText")  # mods正在使用角色
    ...
    def escBack(self,event):
        """按Esc返回上一页"""
        if self.isSkinListPage():
            self.nowPage = "roleList"
            self.cacheThread(threading.Thread(target=self.loadRoleListContent,daemon=True),"content")
